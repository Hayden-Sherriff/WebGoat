...
public class GeneralLessonIntegrationTest extends IntegrationTest {
    ...
    public void httpBasics() {
        ...
        params.put("question_0_solution", "Solution 2: Hyper text transfer protocol");
        params.put("question_1_solution", "Solution 3: To identify the status of a client's request");
        params.put("question_2_solution", "Solution 1: GET");
        params.put("question_3_solution", "Solution 2: False");
        params.put("question_4_solution",
                "Solution 3: To store state on a client's browser across multiple HTTP requests");
        params.put("question_5_solution", "Solution 4: Certificate authorities");
        params.put("question_6_solution", "Solution 4: All of the above");
        checkAssignment(webGoatUrlConfig.url("HttpBasics/quiz"), params, true);

        var result = RestAssured.given()
                .when()
                .relaxedHTTPSValidation()
                .cookie("JSESSIONID", getWebGoatCookie())
                .get(webGoatUrlConfig.url("service/lessonoverview.mvc/%s.lesson".formatted("HttpBasics")))
                .andReturn();

        MatcherAssert.assertThat(
                result.then().statusCode(200).extract().jsonPath()
                        .getList("findAll { it.assignment.name != 'HttpBasicsExternal' }.solved"),
